...
## 子代理路由（2026-09-18 更新，避免重复查询模型列表）

- 当前可用组合：**`workbuddy` / `deepseek-v4.1-flash` / 思考档 `high`**（该模型在 workbuddy 可选档位为 low / high，无 max，取最高 = high）
- 沿革：`buddy` 同模型连续两次子代理无遗言失败（2026-09-18），暂不可用；`codearts` 无 `deepseek-v4.1-flash`。按「模型优先、供应商顺延」规则落到 workbuddy。
