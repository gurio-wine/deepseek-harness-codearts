 ...
 * - `9074`（「当前参与用户太多，请稍后再试」）：⚠️ **定性已于 2026-09-20 推翻**
 *   —— 旧记载「瞬时频次软限流」**不成立**。签到侧三日取证：同账号 09-17/18/19
 *   报错 15 / 187 / 250 次（合计 **452 次**）、**8 秒退避重放仍 9074**、
 *   **同一套设备头下 status 恒成功而 claim 恒 9074**（读/写两个名额桶）。
 *   新定性：**活动级当日容量/名额限制或账号侧风控**。仍留在本表（动作仍是
 *   「退避、不换号」：名额与账号无关，换号只会多烧一个账号的往返），但
 *   **它不记冷却徽章**（见 {@link recordsTraeCnCooldown}），且签到侧的引导文案
 *   按「稍后或次日再试」写（`src/trae-cn-credits.ts` 的 `describeFailureCode`）。
